...
public:
    ...
    Runtime()
    {
        ...
        // TODO: set runtime options before creating the runtime
        //bool abortOnUnhandledRejection, size_t stackSize = 0
        ...
        rt = TJS_GetJSRuntimeFromTJS(qrt);
    }
...
public:
    ...
    public:
        ...
        {
            return add(name, tjs::fwrapper<F>{name});
        }
    ...
    private:
        ...
        class class_registrar
        {
            const char * name;
            tjs::Value prototype;
            tjs::Context::Module& module;
            tjs::Context& context;
        public:
            explicit class_registrar(const char * name, tjs::Context::Module& module, tjs::Context& context) :
                    name(name),
            ...
            class_registrar& constructor(const char * name = nullptr)
            {
                if(!name)
                    name = this->name;
                Value ctor = context.newValue(tjs::ctor_wrapper<T, Args...>{name});
    ...
    public:
        ...
        {
            return class_registrar<T>{name, *this, tjs::Context::get(ctx)};
        }
...
private:
    void init()
    {
        // TODO: this will conflict with tjs
        JS_SetContextOpaque(ctx, this);
        js_traits<detail::function>::register_class(ctx, "C++ function");
    }

public:
    Context(Runtime& rt) : Context(rt.qrt)
    {}

    Context(TJSRuntime * qrt) : modules{ TJS_GetJSRuntimeFromTJS(qrt) }
    {
        ctx = TJS_GetJSContext(qrt);
        if(!ctx)
            throw std::runtime_error{"qjs: Cannot get context"};
        init();
    }
    ...
    Value evalModule(JSContext* ctx, const char* filepath, char* override_filename, int eval_flags)
    {
        JSValue v = TJS_EvalFile(ctx, filepath, eval_flags, true, override_filename);
        return Value{ ctx, v };
    }
